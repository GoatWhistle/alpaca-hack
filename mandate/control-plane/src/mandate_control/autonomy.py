...
class Trajectory(BaseModel):
    """Operator-owned preferences for the autonomous paper-trading loop."""
    ...
    news_poll_seconds: int = Field(default=30, ge=15, le=3600)
    analysis_interval_minutes: int = Field(default=3, ge=1, le=1440)
    ...
    options_confirmation: bool = True
    risk_posture: Literal["defensive", "balanced", "opportunistic"] = "opportunistic"
    thesis: str = Field(
...
class AutonomyStore:
    ...
    def update(
        ...
        *,
        ...
        # The configured symbols are a seed universe. Liquid Alpaca movers can be
        # admitted for the current session by the monitoring pipeline.
        self._write(updated)
        return updated
