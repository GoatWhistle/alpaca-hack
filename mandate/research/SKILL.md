...
6. Research output never authorizes an order. The persistent trader may return only `trade.plan.v2`, referencing opaque IDs from `evaluation.trade_candidates`; deterministic execution then resolves those IDs back to canonical order fields.
...
13. Never infer execution permission from a research score. There is one automatic paper-only path: hard exits bypass the model, while entries require a valid challenged `trade.plan.v2` and pass the deterministic whole-plan risk gate. Operator chat can only request approval-gated append-only memory changes.
