...
6. Research output never authorizes an order. The persistent trader may return only `trade.plan.v3`, referencing opaque IDs from `evaluation.trade_candidates`; its bounded `position_actions` may advise HOLD/REDUCE/EXIT only for live underlyings. Deterministic execution resolves all IDs and quantities.
...
13. Never infer execution permission from a research score. Hard exits bypass the model, while entries and advisory position changes require a valid challenged `trade.plan.v3` and pass deterministic validation. The position watcher is advisory by default. Its optional `INVALIDATED+EXIT` fast lane must be explicitly enabled and still produces only an exit-only PARK plan clamped against live positions. Operator chat can only request approval-gated append-only memory changes.
